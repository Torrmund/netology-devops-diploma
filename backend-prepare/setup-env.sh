...
usage() {
    ...
    echo ""
    echo "Опции:"
    echo "  -c, --credentials PATH      Путь к файлу .aws/credentials"
    echo "  -k, --keyfile PATH          Путь к файлу authorized_key.json"
...
if [ -z "${CREDENTIALS_FILE}" ] || [ -z "${KEYFILE_PATH}" ] || [ -z "${TF_VAR_cloud_id}" ] || [ -z "${TF_VAR_folder_id}" ]; then
...
export TF_VAR_key_file="${KEYFILE_PATH}"
